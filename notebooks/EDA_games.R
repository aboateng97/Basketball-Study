...
games <- readRDS("../data/processed/games_clean.rds")
head(games)
...
hist_points <- ggplot(games, aes(x = point_diff)) +
  ...
  geom_vline(aes(xintercept = mean(point_diff, na.rm = TRUE)), color = "steelblue", linetype = "dashed") +
...
hist_distance <- ggplot(games, aes(x = dist_miles)) +
  geom_histogram(bins = 50, fill = "steelblue", alpha = 0.7) +
  geom_vline(aes(xintercept = mean(dist_miles, na.rm = TRUE)), color = "steelblue", linetype = "dashed") +
...
# Scatterplots of point differential vs key variables (all steelblue)
...
# Linear models
lm1 <- lm(point_diff ~ dist_miles, data = games)
lm2 <- lm(point_diff ~ dist_miles + attendance, data = games)
